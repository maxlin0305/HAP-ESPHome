...
namespace esphome
{
  namespace homekit
  {
      ...
      static int climate_write(hap_write_data_t write_data[], int count, void* serv_priv, void* write_priv) {
        ...
        for (int i = 0; i < count; i++) {
          ...
          if (!strcmp(type, HAP_CHAR_UUID_TARGET_HEATING_COOLING_STATE)) {
            switch (write->val.i)
            {
            case 0: obj->make_call().set_mode(climate::ClimateMode::CLIMATE_MODE_OFF).perform(); break;
            case 1: obj->make_call().set_mode(climate::ClimateMode::CLIMATE_MODE_HEAT).perform(); break;
            case 2: obj->make_call().set_mode(climate::ClimateMode::CLIMATE_MODE_COOL).perform(); break;
            case 3: obj->make_call().set_mode(climate::ClimateMode::CLIMATE_MODE_AUTO).perform(); break;
            default: break;
            }
    ...
    public:
      ...
      void setup_units(TemperatureUnits units) {
        hap_acc_cfg_t acc_cfg{};
        static std::string acc_name_str;
        static std::string acc_serial_str;

        acc_name_str = this->climatePtr->get_name();
        acc_serial_str = std::to_string(this->climatePtr->get_object_id_hash());

        acc_cfg.name = acc_name_str.c_str();
        acc_cfg.serial_num = acc_serial_str.c_str();
        acc_cfg.model = "ESP-CLIMATE";
        acc_cfg.manufacturer = "rednblkx";
        acc_cfg.fw_rev = "0.1.0";
        ...
        if (service) {
          ...
          hap_serv_set_priv(service, strdup(acc_serial_str.c_str()));
          hap_serv_set_write_cb(service, climate_write);
          hap_serv_set_read_cb(service, climate_read);
          hap_acc_add_serv(accessory, service);
          hap_add_bridged_accessory(accessory, hap_get_unique_aid(acc_serial_str.c_str()));
        }
      }
    };
  }
